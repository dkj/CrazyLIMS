...
This document contains the detailed environment, setup, and workflow notes that previously lived in `README.md`. The Phase 1 Redux reboot realigns the stack around a transaction-centric security backbone: every write must occur inside an attributed transaction context, with audit trails and RLS policies deriving actor metadata from shared session settings. For the doc index and navigation, see `README.md`.
...
The dev console includes persona switching, security monitoring views, and the ELN workbench. Legacy sample/inventory pages are still the older placeholder endpoints; treat them as read-only demos until the provenance-backed UI is refreshed.
