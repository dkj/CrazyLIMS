...
- TODO: evaluate bumping the Pyodide wheelhouse to a newer `httpx` that includes `FetchTransport` and swap the Pyodide client helper to `httpx.AsyncClient(transport=httpx.FetchTransport())`; rerun JupyterLite vendor + Playwright to ensure Pyodide still reaches PostgREST.
