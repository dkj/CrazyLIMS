...
- The vendored JupyterLite bundle now carries a wheelhouse under `/eln/lite/pypi` with a Pyodide-ready `crazylims-postgrest-client` (plus `httpx`/`pyodide-http`). The viewer auto-installs it and forwards the active persona token + API base, so notebooks can call PostgREST via `from crazylims_postgrest_client.pyodide import build_authenticated_client`.
- Playwright covers the Python client path (`ui/tests/jupyterlite.spec.ts`) by importing it inside the embedded notebook, asserting the bearer header is present, and exercising the `rpc/actor_accessible_scopes` call through the mocked PostgREST route.
- The Pyodide wheelhouse now bundles the upgraded `httpx` release, and the Pyodide client helper prefers `httpx.FetchTransport` when the runtime exposes it (falling back to `pyodide_http` where needed) so PostgREST calls continue to flow through browser `fetch` after the runtime upgrade.
