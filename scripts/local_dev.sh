...
POSTGREST_VERSION="${POSTGREST_VERSION:-v14.1}"
...
ensure_dbmate() {
  DBMATE_NO_DOCKER=1 LOCAL_DB_PORT="${PG_PORT}" "${ROOT_DIR}/ops/db/bin/dbmate" --help >/dev/null 2>&1 || true
}

run_dbmate() {
  DBMATE_NO_DOCKER=1 LOCAL_DB_PORT="${PG_PORT}" DATABASE_URL="${DATABASE_URL}" "${ROOT_DIR}/ops/db/bin/dbmate" "$@"
}
